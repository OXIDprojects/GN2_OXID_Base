[{block name="widget_product_listitem_grid"}]
    ...
    [{capture name=product_price}]
        ...
        [{/block}]
    [{/capture}]
    <a id="[{$testid}]" href="[{$productLink}]" class="titleBlock title fn" title="[{$product->oxarticles__oxtitle->value}] [{$product->oxarticles__oxvarselect->value}]">
    ...
    [{block name="widget_product_listitem_grid_tobasket"}]
        <div class="priceBlock">
            [{oxhasrights ident="TOBASKET"}]
                ...
                [{if !$blShowToBasket }]
                    <a href="[{$productLink}]" class="toCart button">[{oxmultilang ident="WIDGET_PRODUCT_PRODUCT_MOREINFO"}]</a>
                [{else}]
