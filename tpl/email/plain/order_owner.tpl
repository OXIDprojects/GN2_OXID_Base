...
[{if $payment->oxuserpayments__oxpaymentsid->value == "oxempty"}][{oxcontent ident="oxadminordernpplainemail"}][{else}][{oxcontent ident="oxadminorderplainemail"}][{/if}]
[{/block}]
...
[{if $oViewConf->getShowGiftWrapping()}][{assign var="oWrapping" value=$basketitem->getWrapping()}][{oxmultilang ident="EMAIL_ORDER_CUST_HTML_WRAPPING"}] [{if !$basketitem->getWrappingId()}][{oxmultilang ident="EMAIL_ORDER_CUST_HTML_NONE"}][{else}][{$oWrapping->oxwrapping__oxname->getRawValue()}][{/if}][{/if}]
...
[{block name="email_plain_order_ownernodiscounttotalnet"}][{* netto price *}]
[{oxmultilang ident="EMAIL_ORDER_CUST_HTML_TOTALNET"}] [{$basket->getProductsNetPrice()}] [{$currency->name}]
[{/block}]
[{block name="email_plain_order_ownernodiscountproductvats"}][{* VATs *}]
...
[{/block}]
[{/if}]
[{block name="email_plain_order_ownertotalgross"}][{* brutto price *}]
[{oxmultilang ident="EMAIL_ORDER_CUST_HTML_TOTALGROSS"}] [{$basket->getFProductsPrice()}] [{$currency->name}]
[{/block}]
[{if $basket->getDiscounts()}][{* applied discounts *}]
[{block name="email_plain_order_ownerdiscounts"}]
  ...
  [{/foreach}]
[{/block}]
[{block name="email_plain_order_ownertotalnet"}][{* netto price *}]
  [{oxmultilang ident="EMAIL_ORDER_CUST_HTML_TOTALNET"}] [{$basket->getProductsNetPrice()}] [{$currency->name}]
[{/block}]
[{block name="email_plain_order_ownerproductvats"}][{* VATs *}]
  [{foreach from=$basket->getProductVats() item=VATitem key=key}]
    [{oxmultilang ident="EMAIL_ORDER_CUST_HTML_PLUSTAX1"}] [{$key}][{oxmultilang ident="EMAIL_ORDER_CUST_HTML_PLUSTAX2"}] [{$VATitem}] [{$currency->name}]
  [{/foreach}]
[{/block}]
[{/if}]
[{block name="email_plain_order_ownervoucherdiscount"}][{* voucher discounts *}]
...
[{/if}]
[{/block}]
[{block name="email_plain_order_ownerdelcosts"}][{* delivery costs *}]
[{if $basket->getDelCostVat() > 0}][{* delivery VAT (if available) *}]
...
[{/block}]
[{block name="email_plain_order_ownerpaymentcosts"}][{* payment sum *}]
[{if $basket->getPaymentCosts()}]
[{if $basket->getPaymentCosts() >= 0}][{* payment sum VAT (if available) *}]
  [{oxmultilang ident="EMAIL_ORDER_CUST_HTML_PAYMENTCHARGEDISCOUNT1"}][{else}][{oxmultilang ident="EMAIL_ORDER_CUST_HTML_PAYMENTCHARGEDISCOUNT2"}][{/if}] [{oxmultilang ident="EMAIL_ORDER_CUST_HTML_PAYMENTCHARGEDISCOUNT3"}] [{$basket->getPayCostNet()}] [{$currency->name}]
...
[{block name="email_plain_order_ownergrandtotal"}][{* grand total price *}]
