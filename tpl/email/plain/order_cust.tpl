...
[{if $payment->oxuserpayments__oxpaymentsid->value == "oxempty"}][{oxcontent ident="oxuserordernpplainemail"}][{else}][{oxcontent ident="oxuserorderplainemail"}][{/if}]
[{/block}]
...
[{if $oViewConf->getShowVouchers()}]
    ...
    [{oxmultilang ident="EMAIL_ORDER_CUST_HTML_USEDCOUPONS"}] [{$voucher->oxvouchers__oxvouchernr->value}] - [{oxmultilang ident="EMAIL_ORDER_CUST_HTML_DICOUNT"}] [{$voucherseries->oxvoucherseries__oxdiscount->value}] [{if $voucherseries->oxvoucherseries__oxdiscounttype->value == "absolute"}][{$currency->sign}][{else}]%[{/if}][{/foreach}]
[{/if}]
[{/block}]
...
[{if $payment->oxuserpayments__oxpaymentsid->value == "oxidpayadvance"}]
    ...
    [{oxmultilang ident="EMAIL_ORDER_CUST_HTML_BANK"}] [{$shop->oxshops__oxbankname->value}]
    [{oxmultilang ident="EMAIL_ORDER_CUST_HTML_ROUTINGNOMBER"}] [{$shop->oxshops__oxbankcode->value}]
    [{oxmultilang ident="EMAIL_ORDER_CUST_HTML_ACCOUNTNOMBER"}] [{$shop->oxshops__oxbanknumber->value}]
    [{if $shop->oxshops__oxbiccode->value != ""}][{oxmultilang ident="EMAIL_ORDER_CUST_HTML_BIC"}] [{$shop->oxshops__oxbiccode->value}][{/if}]
