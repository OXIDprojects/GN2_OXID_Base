...
[{block name="sidebar"}]
    ...
    [{oxifcontent ident="oxadvantages" object="oContent"}]
        <div class="box advantages" id="sidebarAdvantages">
            <h3>[{$oContent->oxcontents__oxtitle->value}]</h3>
    ...
    [{block name="sidebar_boxproducts"}]
        [{if $oView->getClassName() eq "start" && $oView->getTop5ArticleList()}]
        	[{include file="widget/product/boxproducts.tpl" listId="topBox" header="" head="BOX_TOPOFTHESHOP_HEADER"|oxmultilangassign products=$oView->getTop5ArticleList() }]
        [{/if}]
    [{/block}]
