[{block name="footer_main"}]
    ...
    [{gn2_oxscript add="$(function(){oxEqualizer.equalHeight($( '#panel dl' ));});"}]
    <div id="footer">
        <div id="panel">
                <div class="bar">
                            ...
                            [{oxifcontent ident="oxdeliveryinfo" object="oCont"}]
                                ...
                                [{if $oView->getUser()}]
                                    ...
                                    [{if $oUser->oxuser__oxustid->value && $oUser->oxuser__oxcompany->value}]
                                        <a class="plainPopUp" href="[{$oCont->getLink()|oxaddparams:'plain=1' }]" rel="nofollow"> [{oxmultilang ident="PLUS_SHIPPING3" }]</a>
                                    [{else}] 
                                        <a class="plainPopUp" href="[{$oCont->getLink()|oxaddparams:'plain=1' }]" rel="nofollow">[{oxmultilang ident="FOOTER_INCLTAXANDPLUSSHIPPING" }]</a>
                                    [{/if}]
                                [{else}]
                                    <a class="plainPopUp" href="[{$oCont->getLink()|oxaddparams:'plain=1' }]" rel="nofollow">[{oxmultilang ident="FOOTER_INCLTAXANDPLUSSHIPPING" }]</a>
                                [{/if}]
...
[{if $oView->isRootCatChanged()}]
    ...
    [{gn2_oxscript add="$( '#scRootCatChanged' ).oxModalPopup({ target: '#scRootCatChanged', openDialog: true});"}]
    <div id="scRootCatChanged" class="popupBox">
