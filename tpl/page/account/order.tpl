...
<h2 class="pageHead accountHead">[{oxmultilang ident="PAGE_ACCOUNT_ORDER_TITLE"}]</h2>
...
<ul class="orderList">
                    ...
                    <td>
                        <dl>
                            <dt title="[{oxmultilang ident="PAGE_ACCOUNT_ORDER_DATE"}]">
                                <strong id="accOrderDate_[{$order->oxorder__oxordernr->value}]">[{php}]setlocale(LC_ALL, 'de_DE');[{/php}] [{$order->oxorder__oxorderdate->value|date_format:"%e. %B %Y"|utf8_encode}]</strong>
                            ...
                            [{foreach from=$order->getOrderArticles(true) item=orderitem name=testOrderItem}]
                                ...
                                <tr id="accOrderAmount_[{$order->oxorder__oxordernr->value}]_[{$smarty.foreach.testOrderItem.iteration}]">
                                  <td>
                                    ...
                                    [{/if }]
                                        <span class="amount">[{$orderitem->oxorderarticles__oxamount->value }] [{*oxmultilang ident="PAGE_ACCOUNT_ORDERHISTORY_QNT"*}]</span> [{$orderitem->oxorderarticles__oxtitle->value}] [{$orderitem->oxorderarticles__oxselvariant->value}]
  ...
  [{else}]
  <p>[{oxmultilang ident="PAGE_ACCOUNT_ORDER_EMPTYHISTORY"}]</p>
  [{/if }]
