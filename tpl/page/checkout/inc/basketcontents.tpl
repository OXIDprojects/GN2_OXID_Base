[{* basket contents *}]
[{gn2_oxscript include="js/widgets/oxbasketchecks.js" priority=10 }]
[{gn2_oxscript add="$('#checkAll, #basketRemoveAll').oxBasketChecks();"}]
[{assign var="currency" value=$oView->getActCurrency()}]


[{if $editable}]
    <form class="basketContent" name="basket[{$basketindex }]" action="[{$oViewConf->getSelfActionLink()}]" method="post">
...
[{/if}]
    ...
    <table id="basket" class="basketitems[{if $oViewConf->getActiveClassName() == 'order' }] orderBasketItems[{/if}]">
        ...
        <thead>
            <tr>
                ...
                <th class="article">[{oxmultilang ident="PAGE_CHECKOUT_BASKETCONTENTS_PRODUCT"}]</th>
                <th></th>
                [{if $oView->isWrapping() }]
                <th class="wrapping">[{oxmultilang ident="PAGE_CHECKOUT_BASKETCONTENTS_WRAPPING"}]</th>
                [{/if}]
                <th class="quantity">[{oxmultilang ident="PAGE_CHECKOUT_BASKETCONTENTS_QUANTITY"}]</th>
                <th class="unitprice">[{oxmultilang ident="PAGE_CHECKOUT_BASKETCONTENTS_UNITPRICE"}]</th>
                <th class="vat">[{oxmultilang ident="PAGE_CHECKOUT_BASKETCONTENTS_TAX"}]</th>
                <th class="totalprice">[{oxmultilang ident="PAGE_CHECKOUT_BASKETCONTENTS_TOTAL"}]</th>
            </tr>
        </thead>
            ...
            [{block name="checkout_basketcontents_basketitem"}]
                ...
                <tr id="cartItem_[{$smarty.foreach.basketContents.iteration}]">
                    ...
                    [{block name="checkout_basketcontents_basketitem_removecheckbox"}]
                        [{if $editable }]
                            <td class="checkbox">
                                <input type="checkbox" class="checkbox" name="aproducts[[{$basketindex }]][remove]" value="1">
                            </td>
                        [{/if}]
                    [{/block}]
                                ...
                                [{if !$basketitem->isBundle() || !$basketitem->isDiscountArticle()}]
                                    ...
                                    [{if $basketproduct->oxarticles__oxisconfigurable->value}]
                                        [{if $basketitem->getPersParams()}]
                                            ...
                                            [{foreach key=sVar from=$basketitem->getPersParams() item=aParam }]
                                                <p class="persParam"><label>[{oxmultilang ident="PAGE_CHECKOUT_BASKETCONTENTS_PERSPARAM" }]</label> <input class="textbox persParam" type="text" name="aproducts[[{$basketindex }]][persparam][[{$sVar }]]" value="[{$aParam }]"></p>
                                            [{/foreach }]
    ...
    <div>
        ...
        [{block name="checkout_basketcontents_basketfunctions"}]
            [{if $editable }]
                <div id="basketFn" class="basketFunctions">
                    ...
                    <input type="checkbox" class="checkbox" name="checkAll" id="checkAll" title="[{oxmultilang ident="PAGE_CHECKOUT_BASKETCONTENTS_SELECT_ALL"}]">
                    <button id="basketRemoveAll" class="submitButton" name="removeAllBtn"><span>[{oxmultilang ident="PAGE_CHECKOUT_BASKETCONTENTS_SELECT_ALL"}]</span></button>
                    <button id="basketRemove" class="submitButton" type="submit" name="removeBtn"><span>[{oxmultilang ident="PAGE_CHECKOUT_BASKETCONTENTS_REMOVE"}]</span></button>
                    <button id="basketUpdate" class="submitButton" type="submit" name="updateBtn"><span>[{oxmultilang ident="PAGE_CHECKOUT_BASKETCONTENTS_UPDATE"}]</span></button>
                </div>
            [{/if}]
        [{/block}]
        ...
        [{block name="checkout_basketcontents_summary"}]
                [{*  basket summary  *}]
                <table id="basketSummary" class="summary[{if $oViewConf->getActiveClassName() == 'order' }] orderSummary[{/if}]">
                    [{if !$oxcmp_basket->getDiscounts() }]

                        [{block name="checkout_basketcontents_nodiscounttotalnet"}]
                            <tr class="nodiscounttotalnet">
                        ...
                        [{block name="checkout_basketcontents_nodiscountproductvats"}]
                            [{foreach from=$oxcmp_basket->getProductVats() item=VATitem key=key }]
                                <tr class="nodiscountproductvats">
                    ...
                    [{block name="checkout_basketcontents_delcosts"}]
                    	[{oxhideblock}]
							<tr class="delcosts">
								...
								<td class="delinfo">[{oxmultilang ident="PAGE_CHECKOUT_BASKETCONTENTS_SHIPPING_CALCULATED_AFTER"}]</td>
							</tr>
                    	[{/oxhideblock}]
