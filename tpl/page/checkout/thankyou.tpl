[{capture append="oxidBlock_content"}]
    ...
    [{block name="checkout_thankyou_main"}]
        ...
        <div id="thankyouPage">
            ...
            <h2 class="pageHead">[{oxmultilang ident="PAGE_CHECKOUT_THANKYOU_TITLE"}]</h2>

            [{block name="checkout_thankyou_info"}]
                <p>[{oxmultilang ident="PAGE_CHECKOUT_THANKYOU_THANKYOU1"}] [{oxmultilang ident="PAGE_CHECKOUT_THANKYOU_THANKYOU2"}] [{$oxcmp_shop->oxshops__oxname->value}]. [{oxmultilang ident="PAGE_CHECKOUT_THANKYOU_REGISTEREDYOUORDERNO1"}] [{$order->oxorder__oxordernr->value}] [{oxmultilang ident="PAGE_CHECKOUT_THANKYOU_REGISTEREDYOUORDERNO2"}]</p>
                [{if !$oView->getMailError() }]
                    <p>[{oxmultilang ident="PAGE_CHECKOUT_THANKYOU_YOURECEIVEDORDERCONFIRM"}]</p>
                [{else}]
                    <p>[{oxmultilang ident="PAGE_CHECKOUT_THANKYOU_CONFIRMATIONNOTSUCCEED"}]</p>
                [{/if}]
                <p>[{oxmultilang ident="PAGE_CHECKOUT_THANKYOU_WEWILLINFORMYOU"}]</p>
            [{/block}]

            [{block name="checkout_thankyou_proceed"}]
                <p>[{oxmultilang ident="PAGE_CHECKOUT_THANKYOU_YOUCANGO"}]
                ...
                [{if $oxcmp_user->oxuser__oxpassword->value }]
                    ...
                    <a id="orderHistory" rel="nofollow" href="[{oxgetseourl ident=$oViewConf->getSelfLink()|cat:"cl=account_order" }]" class="link">[{oxmultilang ident="PAGE_CHECKOUT_THANKYOU_ORDERHISTORY"}]</a>.</p>
                [{/if}]
            [{/block}]
            ...
            [{block name="checkout_thankyou_ts"}]
                [{if $oViewConf->showTs("THANKYOU") && $oViewConf->getTsId() }]
                    ...
                    <h3 class="sectionHead checkoutHead">[{oxmultilang ident="TS_RATINGS"}]</h3>
            ...
            [{block name="checkout_thankyou_partners"}]
                [{if ( $oView->getTrustedShopId()) || $iswebmiles || $oxcmp_shop->oxshops__oxadbutlerid->value ||
                    ...
                    <h3 class="sectionHead checkoutHead">[{oxmultilang ident="PAGE_CHECKOUT_THANKYOU_PARTNERFROM"}]</h3>
            ...
            [{if $oView->showFinalStep()}]
                [{if $oView->getAlsoBoughtTheseProducts()}]
                    <h3 class="blockHead checkoutHead">
...
[{if $oView->showFinalStep()}]
    [{assign var="checkoutStep" value=5}]
[{else}]
    [{assign var="checkoutStep" value=false}]
[{/if}]

[{include file="layout/page.tpl" sidebar="Left" checkoutStep=$checkoutStep}]