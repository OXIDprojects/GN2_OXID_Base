[{capture append="oxidBlock_content"}]
    ...
    [{block name="checkout_payment_main"}]
        ...
        [{block name="change_shipping"}]
            [{if $oView->getAllSets()}]
                <h3 id="deliveryHeader" class="sectionHead checkoutHead">[{if $oView->getAllSetsCnt() > 1 }][{oxmultilang ident="PAGE_CHECKOUT_PAYMENT_SELECTSHIPPING"}][{else}][{oxmultilang ident="PAGE_CHECKOUT_PAYMENT_SELECTEDSHIPPING"}][{/if}]</h3>
        ...
        [{block name="change_payment"}]
            [{gn2_oxscript include="js/widgets/oxpayment.js" priority=10 }]
            [{gn2_oxscript add="$( '#payment' ).oxPayment();"}]
            ...
            <form action="[{$oViewConf->getSslSelfLink() }]" class="js-oxValidate payment" id="payment" name="order" method="post">
                ...
                [{if $oView->getPaymentList()}]
                    <h3 id="paymentHeader" class="sectionHead checkoutHead">[{oxmultilang ident="PAGE_CHECKOUT_PAYMENT_PAYMENT"}]</h3>
...
[{include file="layout/page.tpl" sidebar="Left" checkoutStep=3}]